...
function SpecificTemplatePage() {
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      const response = await axios.get(
        `http://localhost:8007/plantillas/${id}`
      );
  ...
  const timeAgo = formatDistanceToNow(new Date(template.timestamp), { 
    addSuffix: true,
   locale: es 
});
  return (
    <section className="specific-template-page">
      ...
      <main className="template-detail-container">
        ...
        <section className="template-detail-meta">
          ...
          <p className="template-detail-downloads">
            Descargas: {template.descargas}
          </p>
          <p className="template-detail-date">
            Publicado: {timeAgo}
          </p>
